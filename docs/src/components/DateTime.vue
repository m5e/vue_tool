<template>
  <v-container>
    <v-layout row>
      ...
      <v-flex grid-list-md text-center md2>
        <v-card color="blue-grey lighten-2">
          <v-card-text class="px-0 time minutes title">{{ minutes }}</v-card-text>
        </v-card>
      </v-flex>
      <v-flex grid-list-md text-center md2>
        <v-card color="blue-grey lighten-2">
          <v-card-text class="px-0 time seconds title">{{ seconds }}</v-card-text>
        </v-card>
      </v-flex>
    </v-layout>
    <br />
    <h2>{{ this.area }}のお天気</h2>
    <v-container v-for="forcast in this.forecastLists" :key="forcast.dateLabel" class="d-flex md-6">
